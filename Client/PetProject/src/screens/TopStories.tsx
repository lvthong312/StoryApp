...
import { GET_TOP_STORIES_API } from '../services/ApiUrl';
const TopStories = () => {
  const navigation: NavigationProp<ParamListBase> = useNavigation();
  const { data: topStories, isRefreshing, onRefresh, onEndReached, isFetchingNextPage }: IStoryScroll =
    useInfiniteScroll({
  ...
  function keyExtractor(item: number, index: number) {
    return `TopStories_${item}_${index}`;
  }
